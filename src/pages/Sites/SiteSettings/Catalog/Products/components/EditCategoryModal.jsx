...
export default function EditCategoryModal({ open, onClose, onSave, category, parents }) {
  ...
  const [autoSlug, setAutoSlug] = useState(false) // В режиме редактирования по умолчанию выключено
  ...
  // Автоматическая генерация slug из name (только если включено)
  useEffect(() => {
    if (autoSlug && name) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="w-[500px] max-h-[90vh] rounded bg-white p-4 shadow-xl flex flex-col">
        <h3 className="mb-4 text-lg font-medium">Редактировать категорию</h3>
