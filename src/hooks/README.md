...
3. **Fallback логика**: Сначала пытается получить admin-token, затем site-token
4. **Нормализация**: Автоматически извлекает строку токена из разных форматов ответа
5. **Логирование**: Выводит детальные логи в консоль для отладки
