...
import { quickInfoSchema } from '@/config/blockSchemas/quickInfoSchema'
...
export default function QuickInfoEditor({ block, data, onChange, slug }) {
  ...
  useEffect(() => {
    if (data?.custom_appearance && Object.keys(initialAppearance).length === 0) {
      const values = {}
      for (const field of quickInfoSchema) {
  ...
  const handleFieldChange = (key, value) => {
    if (key === 'custom_appearance' && value === true) {
      const initialValues = initBlockAppearanceFromCommon(quickInfoSchema, siteData?.common)
      onChange(prev => {
  ...
  const hasAppearanceChanged = () => {
    ...
    return quickInfoSchema.some(field => {
